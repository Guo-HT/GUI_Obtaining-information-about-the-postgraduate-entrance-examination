...
class YanZhaoSubNum(object):
    ...
    def get_subject_list(self):
        json_response = requests.post('https://yz.chsi.com.cn/zsml/pages/getZy.jsp', headers=self.headers)
        json_content = json_response.content.decode()
        subject_list = json.loads(json_content)
        # print(subject_list)
        return subject_list

    def show_subject_list(self, subject_list):
        print()
        for temp in subject_list:
            print(f"{temp['mc'].ljust(10, '·')}: {temp['dm'].ljust(4, ' ')}", end='  |  ')
            if (subject_list.index(temp) + 1) % 3 == 0:
                print('\n')

    def run(self):
        sub_list = self.get_subject_list()
        self.show_subject_list(sub_list)
...
if __name__ == '__main__':
    ...
    print(' ************************************************************')
    print()
    print(' 从 * 下面 * 找到学科类别编号，程序会将该专业所有开设高校及各个方向写入指定文件中，并保存到程序同级目录下\n')
    print(' .xls 文件直接用Office Excel打开')
    print(' .tsv 文件用记事本打开 或 拖入Excel内打开\n')
